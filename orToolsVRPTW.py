...
def createDistanceMatrix(timetable):
    global totalNumberOfCustomers            
    customers = {}
    customers[0] = (0, 0)  # adding depot
    for ts in timetable:
        for customer in ts.customerList:
            customers[customer.id] = (customer.xCoord, customer.yCoord)
    totalNumberOfCustomers = len(customers)-1  # subtracting 1 because of the depot
    print(customers)
...
def createTimewindows(timetable):
    timeWindows = []
    timeWindows.append((0,1439))
    for slot in timetable:
        for customer in slot.customerList:
            timeWindows.append((slot.start.hour*60+slot.start.minute, slot.end.hour*60+slot.end.minute))
    return timeWindows

def createDataModel(timetable):
    data = {}
    data['distance_matrix'] = createDistanceMatrix(timetable)
    data['time_windows'] = createTimewindows(timetable)
    data['num_vehicles'] = 1
    data['depot'] = 0
    data['penalty'] = 1000
    return data
...
def solve(timetable):
    data = createDataModel(timetable)
